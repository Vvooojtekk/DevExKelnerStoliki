...
namespace KelnerStoliki.Controllers.ApiControllers {
    public class DataGridEmployees1Controller : ApiController {
        

        [HttpGet]
        public HttpResponseMessage Get(DataSourceLoadOptions loadOptions) {
            ICollection<PrzedmiotZamowienia> PrzedmiotyZamowienia = ZamowieniaLista.Lista[0];
        ...
        public HttpResponseMessage Post(FormDataCollection form) {
            ...
            int i = PrzedmiotyZamowienia.Count + 1;
            foreach (var item in PrzedmiotyZamowienia)
            {
                if (i == item.ID)
                {
                    i++;
